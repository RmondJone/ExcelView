       ...
       //如果锁第一行
        if(_isLockFristColumn){
           //如果锁第一列
            ...
            //构造锁定视图
            UILabel *lockView=[[UILabel alloc]initWithFrame:cell.lockView.bounds];
            ...
            for (NSString *data in [self.mXTableDatas objectAtIndex:indexPath.row]) {
                UIView *view=[[UIView alloc]initWithFrame:CGRectMake(x, 0, 70, 45)];
        ...
        if(_isLockFristColumn){
            ...
            if(indexPath.row==0){
                ...
                cell.lockView.layer.backgroundColor=RGB(229, 239, 254).CGColor;
                //构造滚动视图
                CGFloat x=0;
                for (NSString *data in self.mFristRowDatas) {
                    ...
                    view.layer.backgroundColor=RGB(229, 239, 254).CGColor;
                    [cell.scrollView addSubview:view];
                    x+=view.frame.size.width;
                }
                cell.scrollView.contentSize=CGSizeMake(x, cell.scrollView.frame.size.height);
    ...
    if(_isLockFristRow){
        if(_isLockFristColumn){
           //如果第一列锁定
            ...
            CGFloat x=0;
            for (NSString *data in self.mFristRowDatas) {
                UIView *view=[[UIView alloc]initWithFrame:CGRectMake(x, 0, cell.lockView.frame.size.width, 45)];
                UILabel *dataView=[[UILabel alloc]initWithFrame:view.bounds];
                dataView.text=data;
                dataView.textColor=RGB(94, 153, 251);
                dataView.textAlignment=NSTextAlignmentCenter;
